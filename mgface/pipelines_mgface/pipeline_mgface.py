...
# Modified by Mengting Wei for MagicFace
import inspect
...
class MgPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def __init__(
        ...
        unet_deno: UNetDeno2DConditionModel,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if (au is None) or (source is None):
        ...
        bg_latents = self.prepare_id_latents(
            bg_image,
            batch_size,
            num_images_per_prompt,
            prompt_embeds.dtype,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    # print(self.do_classifier_free_guidance, self.image_guidance_scale)
    ...
    def prepare_au(self, au, batch_size, num_images_per_prompt, dtype, device, do_classifier_free_guidance, generator=None):
        au = au.to(device=device, dtype=dtype)
        batch_size = batch_size * num_images_per_prompt

        if batch_size > au.shape[0] and batch_size % au.shape[0] == 0:
            additional_image_per_prompt = batch_size // au.shape[0]
            au_latents = torch.cat([au] * additional_image_per_prompt, dim=0)
        else:
            au_latents = torch.cat([au], dim=0)

        if do_classifier_free_guidance:
            # print('yes, au is copying an unconditional')
            uncond_au_latents = torch.full(au_latents.shape, 0.0).to(device=device, dtype=dtype)
            au_latents = torch.cat([au_latents, uncond_au_latents], dim=0)
        return au_latents
